## HEADER ####
## Who:KK
## What: Machine learning algorithams(Classification)
...
# Probabilities of which type of crops is suitable for cultivation based on the criteria such as 
...
# to addressing food security challenges and reducing the impacts of climate change.
...
## Setup
setwd("C:/Users/DELL/Downloads/archive")
...
# No information rate is a statstical bench mark that is used to evaluate the classification model
# which means prior probabilities of the most frequent classes in the dataset.the accuracy must be
# greater than no information which means making meaningful predictions which is useful for
# practical information.

# Kappa value is the agreement between predicted value and actutal values and it ranges from
# -1 to 1. in general the value below 0.4 indicates poor agreement and and value above 0.8 indicates
# strong agreement.in our case 0.99 which is suggest the strong agreement.

# The McNemar's Test is a statistical test used to determine if there is a significant
# difference between two related proportions. It is often used in cases where the data is paired, 
# such as in a before-and-after study, or when two classifiers are tested on the same dataset
# in our case its NA which means no paired sample tested or compared with the analysis.

# p-value(accuracy > NIR) which is a null hypothesis testing.in our case value is(2.2e-16) less than 
# alpha value(0.05) which means reject null hypotheis which suggest that our model accuracy is
# significant.
...
# which means our model correctly classifies as a apple. while prevalance and detection rate are equal
# prevalence proportion is 0.04545 and correctly detected 0.0454 which is a detection rate.
# it means it detected all our proportions correctly.detection prevalance is the 
# proportion pf predicted positives which is 0.0454 which is accurate. 
# and balanced accuracy is 1 which is perfect classification.
